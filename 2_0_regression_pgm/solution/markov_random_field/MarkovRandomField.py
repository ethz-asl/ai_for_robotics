...
class ImageSegmenter(object):
    ...
    discount = 0.98  # temperature scheduler's factor
    number_of_labels = 6  # number of labels in the image
    number_of_features = 3  # number of features to use
    ...
    def singleton(self, i, j, label):
        potential = 0.0
        prob = max(
            multivariate_normal.pdf(self.features[i][j],
                                    self.label_mean[label],
                                    self.label_covariance[label]), 0.00001)
        potential = -math.log(prob)
        return potential

    def doubleton(self, i, j, label):
        potential = 0.0
        if i != self.image_height - 1:
            if label == self.image_labels[i + 1][j]:
                potential -= self.beta
            else:
                potential += self.beta
        if j != self.image_width - 1:
            if label == self.image_labels[i][j + 1]:
                potential -= self.beta
            else:
                potential += self.beta
        if i != 0:
    ...
    def calculateGlobalEnergy(self):
        ...
        for i in range(self.image_height):
            for j in range(self.image_width):
                singletons += self.singleton(i, j, self.image_labels[i][j])
                doubletons += self.doubleton(i, j, self.image_labels[i][j])
        return singletons + doubletons / 2.0
