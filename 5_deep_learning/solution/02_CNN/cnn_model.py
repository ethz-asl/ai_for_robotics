...
class CNNModel():
  ...
  def __init__(self):
    ...
    self.first_conv2d_layer =  build_conv2d_layer(self.x_image, self.conv1_filter_size, 
						  self.conv1_n_filters, '_conv1')

    self.first_conv2d_layer = tf.nn.max_pool(self.first_conv2d_layer, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME')

    self.first_conv2d_layer = tf.nn.relu(self.first_conv2d_layer)

    # Second convolutional layer.
    self.second_conv2d_layer =  build_conv2d_layer(self.first_conv2d_layer, self.conv2_filter_size,  
						   self.conv2_n_filters, '_conv2')

    self.second_conv2d_layer = tf.nn.max_pool(self.second_conv2d_layer, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME')
    ...
    self.first_fc_layer = build_fc_layer(self.layer_flat, self.fc_size1, '_fc1')
