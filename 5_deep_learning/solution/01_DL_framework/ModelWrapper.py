...
class ModelWrapper():
  ...
  def train(self):
    ...
    while step < self.params.max_training_steps:
      x_batch, y_target_batch = self.getNextTrainingBatch(self.params.training_batch_size)
      loss = self.optimizer.updateStep(self.network, self.loss_function, x_batch, y_target_batch)
