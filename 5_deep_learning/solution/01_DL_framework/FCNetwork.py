...
class FCNetwork():
    ...
    def __init__(self,
        ...
        if len(hidden_layer_specs) > 0:
            ...
            logger.info('  Constructing input layer with dimensions [{},{}]'.
                        format(self.input_dim, hidden_layer_specs[0]['dim']))
            self.layers.append(
                fc_layer.FCLayer(0, hidden_layer_specs[0]['activation'],
                                 self.input_dim, hidden_layer_specs[0]['dim']))

            # Further hidden layers
            for i in range(1, len(self.hidden_layer_specs)):
                logger.info('  Constructing layer {} with dimensions [{},{}]'.
                            format(i, hidden_layer_specs[i - 1]['dim'],
                                   hidden_layer_specs[i]['dim']))
                self.layers.append(
                    fc_layer.FCLayer(i, hidden_layer_specs[i][
                        'activation'], hidden_layer_specs[i - 1]['dim'],
                                     hidden_layer_specs[i]['dim']))

            # Output layer
            logger.info('  Constructing output layer with dimensions [{},{}]'.
                        format(hidden_layer_specs[-1]['dim'], self.output_dim))
            self.layers.append(
                fc_layer.FCLayer(
                    len(self.layers), output_activation, hidden_layer_specs[-1]
                    ['dim'], self.output_dim))
    ...
    def printDimensions(self):
        for idx, l in enumerate(self.layers):
            if idx == len(self.layers) - 1:
                print('Output layer has dimension {}'.format(
                    l.outputDimension()))
            else:
                print('Hidden layer {} has dimension {}'.format(
                    idx + 1, l.outputDimension()))
    ...
    def setParameters(self, param_dict):
        assert len(param_dict.weights) == len(
            self.layers
        ), "Length of provided weights and network weights don't fit ({} vs {})".format(
            len(param_dict.weights), len(self.layers))
        assert len(param_dict.biases) == len(
            self.layers
        ), "Length of provided biases and network biases don't fit ({} vs {})".format(
            len(param_dict.biases), len(self.layers))
    ...
    def gradients(self, x, loss_function, y_target):
        """
        Gradients of the loss function w.r.t. all weights and biases.
        ...
        delta_fused = dCost_dy * self.layers[-1].derivativeActivation(
            layer_evaluations[-1])
        ...
        for layer_idx in np.arange(len(self.layers) - 1, -1, -1):
            ...
            if layer_idx < len(self.layers) - 1:
                logger.debug('Updating backprop. part of gradient of layer {}'.
                             format(layer_idx))
                delta_fused = np.dot(delta_fused, self.layers[
                    layer_idx
                    + 1].W.T) * self.layers[layer_idx].derivativeActivation(
                        layer_evaluations[layer_idx])
            # Due to matrix derivation rules, the layer evaluations need to be multiplied with delta_fused from the front
            gradients.weights.insert(0,
                                     np.dot(layer_evaluations[layer_idx].T,
                                            delta_fused))
