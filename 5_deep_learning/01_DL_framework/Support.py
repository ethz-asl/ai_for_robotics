...
class Variables():
  """
  Variable structure for networks variables / parameters and their gradients
  """
  weights = []
  biases = []
  ...
  def __add__(self, other_variables):
    assert len(self.weights) == len(other_variables.weights), 'Number of weight entries have to match.'
    assert len(self.biases) == len(other_variables.biases), 'Number of bias entries have to match.'
    
    new_p = Variables()
    for w, b, o_w, o_b in zip(self.weights, self.biases, other_variables.weights, other_variables.biases):
