...
def build_conv2d_layer(previous_layer,
  ...
  weights = tf.get_variable("weights"+var_suffix,
                            [filter_size, filter_size, n_channels_previous_layer, n_filters],
                            tf.float32,
                            tf.contrib.layers.xavier_initializer(),
                            tf.contrib.layers.l2_regularizer(regularizer_weight))
  
  biases  = tf.get_variable("biases"+var_suffix, 
                            n_filters,
                            tf.float32,
                            tf.constant_initializer(0))
  
  conv_layer = tf.nn.conv2d(previous_layer, weights, [1, stride_length, stride_length, 1], 'SAME')
